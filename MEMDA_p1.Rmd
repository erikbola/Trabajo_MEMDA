...
X <- datos[idx_targets_orig, varNum, drop = FALSE]
p <- ncol(X)
...
datos[idx_targets_orig, varNum] <- X_no_outliers
...
```{r}
datos_prepro<-datos_final[,c("liveness_boxcox", "acousticness_boxcox", "danceability", "audio_valence")]
...
# --- 2) NUMÉRICAS: detección y marcaje de outliers ---
X <- test[idx_targets_orig, varNum, drop = FALSE]
p <- ncol(X)
...
# --- 4) Escribir de vuelta las numéricas sin outliers ---
stopifnot(nrow(X_no_outliers) == length(idx_targets_orig))
test[idx_targets_orig, varNum] <- X_no_outliers

cat("\n✅ Outliers numéricos (IQR) y categóricos (niveles raros) marcados como NA en `datos`.\n")
cat("Total de NA's ahora:", sum(is.na(test)), "\n")
...
```{r}
test_prepro<-test_final[,c("liveness_boxcox", "acousticness_boxcox", "danceability", "audio_valence")]
```
...
#submission3 <- data.frame(id = test_num$ID, "song_popularity" = yp_comp)

# Exportar a CSV (sin comillas ni índice)
#write.table(submission3, 
