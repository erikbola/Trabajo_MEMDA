...
# --- 1) Outliers multivariantes KNN (como ya tenías) ---
res_knn <- do_knno(datos[, varNum], k = 1, top_n = 3000)
idx_targets <- res_knn

# --- 2) NUMÉRICAS: mismo flujo que tu código ---
X <- datos[, varNum, drop = FALSE]
p <- ncol(X)
...
outlier_counts <- sapply(X, detect_outliers, method = "iqr")
outlier_vars <- names(outlier_counts[outlier_counts > 0])

cat("Variables numéricas con outliers detectados:\n"); print(outlier_vars)
cat("\nNúmero de outliers por variable (numéricas):\n"); print(outlier_counts[outlier_counts > 0])
...
if (length(idx_targets) == 0) stop("Ningún índice válido en idx_targets; revisa res_knn o el dataset.")

# --- 3) CATEGÓRICAS: detectar niveles raros y marcarlos NA ---
# Definimos "outlier categórico" como nivel raro por baja frecuencia global.
# Puedes ajustar min_prop y min_count según tu tamaño muestral.
min_prop  <- 0.01   # <1% de los casos
min_count <- 20     # o menos de 20 observaciones

# Aseguramos tipo factor (recomendado)
for (v in varCat) {
  if (is.character(datos[[v]])) datos[[v]] <- factor(datos[[v]])
}

# Calcular niveles raros por variable categórica (global)
rare_levels_list <- lapply(varCat, function(v) {
  x <- datos[[v]]
  if (!is.factor(x)) x <- factor(x)
  if (nlevels(x) <= 1) return(character(0))
...
# Marcar como NA los niveles raros (global, igual que con numéricas: no usamos idx_targets aquí)
for (v in varCat) {
  rare_lvls <- rare_levels_list[[v]]
  if (length(rare_lvls) == 0) next
  datos[[v]][datos[[v]] %in% rare_lvls] <- NA
}

# --- 4) Escribir de vuelta las numéricas sin outliers en datos ---
datos[, varNum] <- X_no_outliers

cat("\n✅ Outliers numéricos (IQR) y categóricos (niveles raros) marcados como NA en `datos`.\n")
cat("Total de NA's ahora: ", sum(is.na(datos)), "\n")
```
...
El **análisis** de **componentes principales** sobre las **7 variables óptimas** revela una estructura dimensional bien definida. Los primeros **5 componentes** explican individualmente **más del 10% de la varianza**, acumulando **83.4%** de la **información total**.

PC1 (23.6%) emerge como la dimensión principal, capturando casi un cuarto de la variabilidad del conjunto de datos. La **distribución gradual** de la **varianza** entre los componentes indica que **ninguna** **variable** **domina** excesivamente el **análisis**, sino que **todas** **contribuyen** de manera **balanceada** a las diferentes dimensiones latentes.
...
```{r}
datos_prepro<-datos_final[,c("speechiness", "liveness_boxcox", "acousticness_boxcox", "danceability", "audio_valence")]
...
```{r}
modelo_knn <- knn.reg(train = X_train, test = X_test, y = y_train, k = 19)
yp <- modelo_knn$pred
y  <- y_test
...
# --- RMSE final ---
rmse <- sqrt(mean(e1))
cat("RMSE del modelo con k =", 19, ":", rmse, "\n")
```
...
# --- 1) Outliers multivariantes KNN (como ya tenías) ---
res_knn <- do_knno(test[, varNum], k = 1, top_n = 3000)
idx_targets <- res_knn

# --- 2) NUMÉRICAS: mismo flujo que tu código ---
X <- test[, varNum, drop = FALSE]
p <- ncol(X)
...
for (v in varCat) {
  rare_lvls <- rare_levels_list[[v]]
  if (length(rare_lvls) == 0) next
  test[[v]][test[[v]] %in% rare_lvls] <- NA
}

# --- 4) Escribir de vuelta las numéricas sin outliers en datos ---
test[, varNum] <- X_no_outliers
...
```{r}
test_prepro<-test_final[,c("speechiness", "liveness_boxcox", "acousticness_boxcox", "danceability", "audio_valence")]
```
...
submission <- data.frame(id = test_num$ID, "song_popularity" = yp)
