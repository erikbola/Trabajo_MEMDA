...
default_idx <- sample(nrow(datos_no0), nrow(datos_no0) * 0.7)

train <- datos_no0[default_idx, ]
test  <- datos_no0[-default_idx, ]
```
...
caret::postResample(round(predicciones,0), test$song_popularity)
```
```{r}
error<-abs(round(predicciones,0)-test$song_popularity)
...
accuracy(round(predicciones,0), test$song_popularity)
```
...
```{r}
test_prepro<-test_final[,c("liveness_boxcox", "acousticness_boxcox", "danceability", "audio_valence","outlier")]
```
