...
El **análisis** de **componentes principales** sobre las **8 variables óptimas** revela una estructura dimensional bien definida. Los primeros **5 componentes** explican individualmente **más del 10% de la varianza**, acumulando **84.9%** de la **información total**.

PC1 (30.9%) emerge como la dimensión principal, capturando casi un cuarto de la variabilidad del conjunto de datos. La **distribución gradual** de la **varianza** entre los componentes indica que **ninguna** **variable** **domina** excesivamente el **análisis**, sino que **todas** **contribuyen** de manera **balanceada** a las diferentes dimensiones latentes.

Este resultado confirma que las **8 variables seleccionadas** por **RFE** contienen información complementaria y valiosa para el modelo, justificando la retención de múltiples componentes que capturan distintas facetas de las características musicales relevantes para predecir la popularidad.
...
Creamos ahora para datos_backup
```{r}
# Crear variable binaria para el modelo de clasificación
datos_backup$is_zero <- ifelse(datos_backup$song_popularity == 0, 1, 0)

# Dividmos en train y test
train_backup <- subset(datos_backup[train_index_full, ], 
                       select = -c(ID,outlier,audio_mode,key,liveness,time_signature))
test_backup  <- subset(datos_backup[-train_index_full, ], 
                       select = -c(ID,outlier,audio_mode,key,liveness,time_signature))

# La única transformación que mejora el modelo es la de acousticness
train_backup$acousticness<-train_full$acousticness_boxcox
test_backup$acousticness<-test_full$acousticness_boxcox

# Entrenar un modelo clasificador para predecir ceros
library(randomForest)

modelo_clasif_backup <- randomForest(
  as.factor(is_zero) ~ . -song_popularity,
  data = train_backup,
  ntree = 300
)
...
# Obtener probabilidades en test
test_backup$prob_zero <- predict(modelo_clasif_backup, newdata = test_backup, type = "prob")[,2]

# Guardamos el datasets definitivos.
train_backup<-subset(train_backup, select=-is_zero)
test_backup<-subset(test_backup, select=-is_zero)
datos_backup<-subset(datos_backup, select=-is_zero)
```
...
Además, después de haber hecho nuevas pruebas modelando con random forest, el modelo que mejores resultados nos ha proporcionado, nos hemos dado cuenta que este modelo funciona mejor otorgandole más variables por mucho que mantengan correlación moderada entre algunas de ellas. A esta bbdd le llamaremos datos_backup y, después de haber testeado el efecto de la inclusión de todas las variables, el conjunto seleccionado de este dataset y que mejor rendimiento a proporcionado és: *loudness, danceability, acousticness_boxcox,energy,audio_valence,tempo,song_duration_ms,speechiness, instrumentalness*. 

Cabe destacar, que anteriormente habiamos extraido de nuestra selección de atributos algunas de las variables recien mencionadas por problemas de variabilidad reducida, correlación o asimetria. No obstante, en este trabajo priorizamos la capacidad predictiva en frente de la explicativa y concluimos que estos problemas no causan ningun efecto preocupante en el devenir de los modelos.

```{r}
#save(datos, train, test, 
#     datos_full, train_full, test_full, 
#     datos_backup, train_backup, test_backup,
#     file = "Datos_Prepocesados.RData")
...
for (i in seq_along(k_values)) {
  ...
  pred <- knn.reg(train=subset(train_backup,select=-song_popularity), 
                  test=subset(test_backup ,select=-c(song_popularity,prob_zero)),
                  y = train_backup$song_popularity, 
                  k = k)$pred
  
  results$RMSE[i] <- rmse(test_backup$song_popularity, pred)
  results$MAPE[i] <- mape(test_backup$song_popularity, pred)
}
...
```
Dado que el valor de k que minimiza el **RMSE** es 19, y el que minimiza el **MAPE** es 1, se procede a calcular el modelo con k=19 debido a que este és el inidcador guia de nuestra trabajo.
...
```{r}
modelo_knn <- knn.reg(train= subset(train_full,select=-song_popularity), 
                  test= subset(test_full,select=-c(song_popularity,prob_zero)),
                  y = train_full$song_popularity, 
                  k = 19)
yp <- modelo_knn$pred
y  <- test_full$song_popularity

# --- RMSE final ---
#rmse <- sqrt(mean(e1))
cat("RMSE del modelo con k =", 17, ":", rmse(y,yp), "\n")
```
Hemos probado aplicar el modelo a nuestras tres bbdd y ha funcionado mejor con datos_full. De esto concluimos que el knn funciona mejor con menos variables, es decir, recompensa la parsiomonia.
...
```
Probamos aplicar cart en las tres bbdd y vemos con cual rinde mejor.
```{r}
library(rpart)
library(rpart.plot)
library(caret)
...
umbral<-0.5
# Predicciones
pred_train <- predict(modelo_rpart, newdata = train_backup)
pred_test  <- predict(modelo_rpart, newdata = test_backup)
pred_test<- ifelse(test_backup$prob_zero > umbral,
                          0,          
                          pred_test)

# Métricas
accuracy(pred_train, train_backup$song_popularity)
(kpi_cart<-accuracy(pred_test,  test_backup$song_popularity))
...
```
El mejor modelo ha sido el aplicado sobre la bbdd repreprocesada priorizando la capacidad predictiva sobre la explicativa. 
...
pred_RF_full <- ifelse(test_full$prob_zero > umbral,
                          0,          
                          pred_reg_full) 
accuracy(pred_RF_full, test_full$song_popularity)
```
...
#print(modelo_full)
predicciones_backup <- predict(modelo_backup,data = test_backup)
importancia_pred_backup <- modelo_backup$variable.importance
sort(importancia_pred_backup,decreasing=TRUE)
```
KPI's
```{r}
pred_reg_backup<-predicciones_backup$predictions
pred_RF_backup <- ifelse(test_backup$prob_zero > umbral,
                          0,          
                          pred_reg_backup) 
(kpi_RF<-accuracy(pred_RF_backup, test_backup$song_popularity))
```
Hemos probado otro tipo de modelo mixto pero el modelo no mejora de todos modos.
```{r}
pred_reg_backup_r<-predicciones_backup$predictions
pred_RF_backup_r <- (1-test_backup$prob_zero)*pred_reg_backup_r
accuracy(pred_RF_backup_r, test_backup$song_popularity)
```

#### 10.1.2.4. RF probando escalando
```{r}
# Identificar variables a escalar
vars_scale_train <- setdiff(colnames(train_backup), "song_popularity")
vars_scale_test  <- setdiff(colnames(test_backup),  c("song_popularity", "prob_zero"))

# 2. Calcular media y sd en TRAIN
means_train <- sapply(train_backup[, vars_scale_train], mean, na.rm = TRUE)
sds_train   <- sapply(train_backup[, vars_scale_train], sd,   na.rm = TRUE)

# 3. Escalar TRAIN
train_scaled <- train_backup
train_scaled[, vars_scale_train] <-
  scale(train_backup[, vars_scale_train],
        center = means_train,
        scale  = sds_train)

# 4. Escalar TEST usando parámetros del TRAIN
test_scaled <- test_backup
test_scaled[, vars_scale_test] <-
  scale(test_backup[, vars_scale_test],
        center = means_train[vars_scale_test],
        scale  = sds_train[vars_scale_test])

```

```{r}
# Medias ~ 0 y sd ~ 1 en TRAIN
colMeans(train_scaled[, vars_scale_train])
apply(train_scaled[, vars_scale_train], 2, sd)

# Confirmar que NO se han escalado
summary(train_scaled$song_popularity)
summary(test_scaled$prob_zero)
```

Ajuste del modelo con cada combinación
```{r, include=FALSE}
oob_error = rep(NA, nrow(param_grid))
...
resultados_scaled <- param_grid
resultados_scaled$oob_error <- oob_error
resultados_scaled <- resultados_scaled %>% arrange(oob_error)
resultados_scaled
head(resultados_scaled[,4])
p<-which(resultados_scaled[,4]==min(resultados_scaled[,4]))
resultados_scaled[p,]
```
...
predicciones_scaled <- predict(modelo_scaled,data = test_scaled)
importancia_pred_scaled <- modelo_scaled$variable.importance
sort(importancia_pred_scaled,decreasing=TRUE)
```

KPI's

```{r}
pred_reg_scaled<-predicciones_scaled$predictions
pred_RF_scaled <- ifelse(test_scaled$prob_zero > umbral,
                          0,          
                          pred_reg_scaled) 
accuracy(pred_RF_scaled, test_scaled$song_popularity)
```

Vemos que el modelo escalando las variables predictoras sigue sin mejorar.

#### 10.1.2.5. RF output transformado
```{r}
train_transf<-train_backup
test_transf<-test_backup
train_transf$song_popularity<-log1p(train_transf$song_popularity)
```

```{r}
set.seed(123)
modelo_transf  <- ranger(
  formula   = song_popularity ~ .,
  data      = train_transf,
  num.trees = 5000,
  mtry=2,
...
#print(modelo_full)
predicciones_transf <- predict(modelo_transf,data = test_transf)
importancia_pred_transf <- modelo_transf$variable.importance
sort(importancia_pred_transf,decreasing=TRUE)
```
KPI's
```{r}
pred_reg_transf<-expm1(predicciones_transf$predictions)
pred_RF_transf<- ifelse(test_transf$prob_zero > 0.5,
                          0,          
                          pred_reg_transf) 
accuracy(pred_RF_transf, test_transf$song_popularity)
```
El modelo no mejora con la transformación del output.

#### 10.1.2.6. RF backup redondeando
```{r}
set.seed(123)
modelo_red  <- ranger(
  formula   = song_popularity ~ .,
...
#print(modelo_full)
predicciones_red <- predict(modelo_red,data = test_backup)
importancia_pred_red <- modelo_red$variable.importance
sort(importancia_pred_red,decreasing=TRUE)
```
KPI's
```{r}
pred_reg_red<-round(predicciones_red$predictions,0)
pred_RF_red <- ifelse(test_backup$prob_zero > umbral,
                          0,          
                          pred_reg_red) 
accuracy(pred_RF_red, test_backup$song_popularity)
```
El rmse no baja internamente redondeando el vector de predicciones.
...
Probamos modelar sobre las 3 bbdd. (se muestra el codigo con el mejor rendimiento de los 3 trazados internamente)
```{r}
library(caret)
set.seed(1234)
trControl<-trainControl(method="cv", number=5)
caret.gbm0 <- train(song_popularity ~ ., method = "gbm", data = train_backup,
...
caret.gbm1 <- train(song_popularity ~ ., method = "gbm", data = train_backup,
                    tuneGrid = tuneGrid, trControl = trControl, verbose = FALSE)
caret.gbm1
```
Predicciones
```{r}
pred_boost_caret <- predict(caret.gbm1, newdata = test_backup)
pred_boost_caret <- ifelse(test_backup$prob_zero > umbral,
                          0,          
                          pred_boost_caret)
(kpi_boost<-accuracy(pred_boost_caret, test_backup$song_popularity))
```
### 10.2.3. Xboost
```{r}
library(caret)
caret.xgb <- train(song_popularity ~ ., method = "xgbTree", 
                   data = train_backup, trControl = trControl, verbosity = 0)
caret.xgb
caret.xgb$bestTune
```
Predicciones
```{r}
pred_xgboost<-predict(caret.xgb, newdata=test_backup)
pred_xgboost <- ifelse(test_backup$prob_zero > umbral,
                          0,          
                          pred_xgboost)
accuracy(pred_xgboost, test_backup$song_popularity)
```
...
pred_svm_2 <- predict(modelo_svm_opt_2, newdata = test_backup)
pred_svm_2 <- ifelse(test_backup$prob_zero > umbral,
                  0,          
                  pred_svm_2)
# Calculamos los KPIs finales
(kpi_SVM<-accuracy(pred_svm_2, test_backup$song_popularity))
```
...
```{r}
plot(test_backup$song_popularity, pred_svm_2,
...
Hemos obtenido estos resultados de cada una de las 3 bases de datos disponible (datos_full, datos, datos_backup) en cada uno de los modelos. 
```{r}
library(dplyr)
library(gt)

res_rmse <- tibble(
  Modelo = c("KNN", "CART", "RF", "BOOSTING", "SVM"),
  Full   = c(21.9302, 21.6173, 21.0822, 21.4933, 21.7712),
  Datos  = c(21.9734, 21.8506, 21.1420, 21.5290, 21.7215),
  Backup = c(22.0245, 21.3490, 20.5327, 20.9921, 21.3644)
)

tabla_rmse <- res_rmse %>%
  gt() %>%
  tab_header(
    title = md("**Comparación de Modelos Predictivos**"),
    subtitle = "RMSE en Full / Datos / Backup"
  ) %>%
  tab_spanner(
    label = "RMSE",
    columns = c(Full, Datos, Backup)
  ) %>%
  fmt_number(
    columns = c(Full, Datos, Backup),
    decimals = 4
  ...
  # Resaltar el mínimo (mejor) de cada columna
  tab_style(
    style = cell_fill(color = "#E8F5E9"),
    locations = list(
      cells_body(columns = Full,   rows = Full   == min(Full)),
      cells_body(columns = Datos,  rows = Datos  == min(Datos)),
      cells_body(columns = Backup, rows = Backup == min(Backup))
    )
  )
...
```
Vemos que en knn el mejor rendimiento lo ha dado el modelo en el que se ha usado la selección de variables inicial salida del preprocesamiento, hecho que evidencia la importancia que le da este metoddo a la parsimonia del dataset. 

Por otro lado, para el resto de modelos, la bbdd con la selección final de variables priorizando la capacidad predictiva ha rendido por encima de las otras dos. 

A parte, en los 5 casos no se han quitado los outliers, con lo que concluimos que su presencia no afecta negativamente en la calidad de las predicciones de los algoritmos
...
Selección final
```{r}
TEST<-TEST_final[,c("speechiness","liveness_boxcox", "acousticness_boxcox", "danceability", "audio_valence","key3","outlier")]
TEST$song_popularity<-TEST_backup$song_popularity
```

Creacion bbdd con y sin outliers
```{r}
TEST_full <- TEST
TEST <- subset(TEST, outlier == 0)
TEST <- subset(TEST, select=-outlier)
```

Creación de la prob_zero para TEST
```{r}
# Crear variable binaria para el modelo de clasificación
datos$is_zero <- ifelse(datos$song_popularity == 0, 1, 0)
...
Creación de la prob_zero para TEST_full
```{r}
# Crear variable binaria para el modelo de clasificación
datos_full$is_zero <- ifelse(datos_full$song_popularity == 0, 1, 0)
...
modelo_clasif_full <- randomForest(
  as.factor(is_zero) ~ .,
  data = subset(datos_full, select=-song_popularity),
  ntree = 300
)
...
Creación de la prob_zero para TEST_backup. Llamaremos _BACKUP para no sobreescribir sobre _backup.
```{r}
# Seleccionamos las variables 
datos_BACKUP <- subset(datos_backup, 
                       select = -c(ID,outlier,audio_mode,key,liveness,time_signature))
TEST_BACKUP  <- subset(TEST_backup, 
                       select = -c(ID,outlier,audio_mode,key,liveness,time_signature))

# La única transformación que mejora el modelo es la de acousticness
datos_BACKUP$acousticness<-datos_full$acousticness_boxcox
TEST_BACKUP$acousticness<-TEST_full$acousticness_boxcox
...
modelo_clasif_BACKUP <- randomForest(
  as.factor(is_zero) ~ .,
  data = subset(datos_BACKUP, select=-song_popularity),
  ntree = 300
)
...
```{r}
modelo_knn_TEST <- knn.reg(train = subset(datos_full, select=-song_popularity),
                           test = subset(TEST_full, select=-prob_zero), 
                           y = datos_full$song_popularity, 
                           k = 19)
...
Arbol básico
```{r}
tree_rpart <- rpart(song_popularity ~ ., data = datos_BACKUP, method = "anova")
summary(tree_rpart)
...
Predicciones
```{r}
pred_cart  <- predict(tree_opt, newdata = TEST_BACKUP)
```
...
head(param_grid)
```
Ajustamos un modelo con cada combinación de la grid (hecho una vez solo debido a coste computacional alto)
```{r}
#oob_error = rep(NA, nrow(param_grid))
...
resultados[p,]
```
Bosque óptimo (num_trees= 5000, mtry=3, max_depth=20)
```{r}
set.seed(123)
modelo  <- ranger(
  formula   = song_popularity ~ .,
...
#print(modelo)
predicciones <- predict(modelo,data = TEST_BACKUP)
importancia_pred <- modelo$variable.importance
sort(importancia_pred,decreasing=TRUE)
```
...
```{r}
pred_reg<-predicciones$predictions
RF <- ifelse(TEST_BACKUP$prob_zero > umbral,
                          0,          
                          pred_reg) 
```
...
submission13 <- data.frame(ID = TEST_num$ID, "song_popularity" = round(RF,0))

# Exportar a CSV (sin comillas ni índice)
write.table(submission13, 
            file = "submission13.csv", 
...
set.seed(1234)
trControl<-trainControl(method="cv", number=5)
caret.gbm0 <- train(song_popularity ~ ., method = "gbm", data = datos_BACKUP,
                    trControl = trControl, verbose = FALSE)
caret.gbm0
```
...
tuneGrid <- data.frame(n.trees =  100, interaction.depth = 3, 
                       n.minobsinnode = 10, shrinkage = c(0.3, 0.1, 0.05, 0.01, 0.005))
caret.gbm1 <- train(song_popularity ~ ., method = "gbm", data = datos_BACKUP,
                    tuneGrid = tuneGrid, trControl = trControl, verbose = FALSE)
caret.gbm1
```
...
Predicciones
```{r}
pred_boost <- predict(caret.gbm1, newdata = TEST_BACKUP)
```
...
pred_svm_TEST <- predict(modelo_svm_opt_TEST, newdata = TEST_BACKUP)
pred_svm_TEST <- ifelse(TEST_BACKUP$prob_zero > umbral,
                  0,          
                  pred_svm_TEST)
```